...
st.write("তোমার বইয়ের পৃষ্ঠাগুলো আপলোড করে রাখো, তারপর যেকোনো অংক স্ক্যান করলেই পৃষ্ঠা ও লেসন নম্বর পেয়ে যাবে!")

# সাইডবারে API Key নেওয়ার ঘর
st.sidebar.header("⚙️ সেটআপ")
api_key = st.sidebar.text_input("Gemini API Key লিখুন:", type="password")

if api_key:
    genai.configure(api_key=api_key)
    # দ্রুত ও ফ্রি মডেল বেছে নেওয়া হলো
    model = genai.GenerativeModel('gemini-2.5-flash')

    st.sidebar.markdown("---")
    st.sidebar.header("১. বইয়ের পৃষ্ঠা আপলোড")
    uploaded_book_pages = st.sidebar.file_uploader(
        "বইয়ের পৃষ্ঠাগুলোর ছবি তুলুন ও আপলোড করুন:", 
        type=["png", "jpg", "jpeg"], 
    ...
    if st.button("🔍 অংকটি খুঁজে বের করো", type="primary"):
        if not uploaded_book_pages:
            st.error("⚠️ দয়া করে সাইডবার থেকে আগে বইয়ের অন্তত ১টি পৃষ্ঠার ছবি আপলোড করুন!")
        elif not query_image:
            st.error("⚠️ দয়া করে যে অংকটি খুঁজতে চান তার ছবি আপলোড করুন!")
        else:
            with st.spinner("AI আপনার বইয়ের পৃষ্ঠাগুলো স্ক্যান করছে... অনুগ্রহ করে অপেক্ষা করুন।"):
                try:
                    # প্রম্পট তৈরি
                    prompt = """
                    তুমি একজন দক্ষ সহকারী শিক্ষক। 
                    তোমাকে নিচে কিছু বইয়ের পৃষ্ঠা এবং শেষে একটি নির্দিষ্ট অংকের ছবি দেওয়া হয়েছে।
